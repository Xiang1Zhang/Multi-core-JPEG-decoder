...
int main (void)
{
  ...
  t1 = hw_tifu_systimer_get(); 
    ...
    while(1)
    {
		...
		fifo_pull(fcb_from2);
        memcpy( pbout_2, sp_from2->pbout, 10*sizeof(PBlock));
		fifo_release_space(fcb_from2);

        cc( &local.sh1_to1, &(pbout[0]), &cbout);
        raster( &local.sh2_to1, &cbout );

        if( local.sh2_to1.leftover != 0 ) {
            cc( &local.sh1_to2, &(pbout_2[0]), &cbout);
            raster( &local.sh2_to2, &cbout );
        }

        if ( local.sh2_to1.leftover == 0 || local.sh2_to2.leftover == 0 ){
            break;
        }
    }
